...
export async function POST(req: NextRequest) {
  return withWorkspaceAuth(req, async ({ user, workspaceId, membership }) => {
    try {
      ...
      const {
        userId, // Customer/end-user ID (not the authenticated workspace member)
        channel = 'web',
        metadata = {},
      } = body;
      ...
      console.log('Creating conversation:', {
        ...
        customerId: userId,
        channel,
        createdBy: user.id, // Authenticated workspace member
          ...
          if (admins && admins.length > 0) {
            ...
            admins.forEach(async (admin) => {
              try {
                ...
                const { data: { user: adminUser }, error: userError } = await supabaseAdmin.auth.admin.getUserById(
                  admin.user_id
                );
